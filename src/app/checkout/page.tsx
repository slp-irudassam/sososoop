...
import { CHECKOUT_PRODUCTS } from '@/lib/checkout';
...
export default function CheckoutPage() {
  return (
    <Suspense
      fallback={<main className="min-h-[70vh] flex items-center justify-center text-ink-muted">불러오는 중…</main>}
    >
      <CheckoutInner />
    </Suspense>
  );
...
function CheckoutInner() {
  ...
  async function handlePay() {
    ...
    try {
      ...
      const orderId = `${product.slug}-${crypto.randomUUID()}`;
      await payment.requestPayment({
        method: 'CARD',
        amount: { currency: 'KRW', value: product.amount },
        orderId,
        orderName: product.orderName,
        successUrl: `${window.location.origin}/payments/success?product=${product.slug}`,
        failUrl: `${window.location.origin}/payments/fail?product=${product.slug}`,
